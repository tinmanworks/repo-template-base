...
Modes:
  core      Run blocking template checks
...
check_placeholders() {
  ...
  filtered=()
  matches="$(cd "$ROOT_DIR" && rg -n "(Describe what|Key capability [0-9]+|Item [0-9]+|Short description of the project)" README.md docs/overview.md docs/architecture.md docs/adr CONTRIBUTING.md 2>/dev/null || true)"
  if [[ -n "$matches" ]]; then
    while IFS= read -r line; do
      [[ -z "$line" ]] && continue
